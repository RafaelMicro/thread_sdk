...
extern void rt58x_alarm_wakeup_check();
...
void Lpm_Enter_Low_Power_Mode(void)
{
    ...
    if (low_power_mask_status == LOW_POWER_NO_MASK)
    {
        ...
        {
            uint32_t timeout = 0;
            sys_get_retention_reg(4,&timeout);
            //info("ct %u \n" ,(Timer_Current_Get(4)/40));
            enter_critical_section();
            // if(((Timer_Current_Get(4)/40) <= timeout && (Timer_Current_Get(4)/40) > 5)) 
            // {   
            ...
            //     info("np \n");
            // }
            // if((Timer_Current_Get(4)/40) > timeout)
            // {
            //     rt58x_alarm_wakeup_check();
            // }
