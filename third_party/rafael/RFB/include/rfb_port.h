...
void rfb_port_subg_init(rfb_interrupt_event_t *_rfb_interrupt_event, rfb_keying_type_t keying_mode);
bool rfb_port_subg_is_channel_free(uint32_t rf_frequency, uint8_t rssi_threshold);
void rfb_port_subg_rx_config_set(uint8_t data_rate, uint16_t preamble_len, fsk_mod_t mod_idx, crc_type_t crc_type,
                                 whiten_enable_t whiten_enable, uint32_t rx_timeout, bool rx_continuous, uint8_t filter_type);
void rfb_port_subg_tx_config_set(tx_power_level_t tx_power, uint8_t data_rate, uint16_t preamble_len, fsk_mod_t mod_idx,
