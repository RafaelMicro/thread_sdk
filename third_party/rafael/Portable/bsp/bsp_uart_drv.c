...
static uint32_t bsp_uart_rx_line_int_handler(const uart_handle_t *uart_dev)
{
    uint32_t     lsr, event;
...
static void bsp_uart_irqhandler(uint32_t target_id)
{
    int     rval = 0;
    const uart_handle_t  *uart_dev;
    UART_T               *uart;
    ...
    do
    {
        ...
        if ((iir & IIR_INTSTATUS) == 0)
        {
            ...
            {
                event |= bsp_uart_rx_line_int_handler(uart_dev);
                if(event & UART_EVENT_RX_BREAK)
                {
                    Lpm_Low_Power_Mask(LOW_POWER_MASK_BIT_RESERVED30);                    
                }
            }
        }
...
void bsp_uart_disable(uint32_t target_id)
{
    const uart_handle_t  *uart_dev;
    UART_T               *uart;
    uart_dev = &m_uart_handle[target_id];
...
void bsp_uart_enable(uint32_t target_id)
{
    const uart_handle_t  *uart_dev;
    UART_T               *uart;
    uart_dev = &m_uart_handle[target_id];
